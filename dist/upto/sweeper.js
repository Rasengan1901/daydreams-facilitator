...
import { settleUptoSession } from "./settlement.js";
export function createUptoSweeper(config) {
        ...
        for (const [id, session] of config.store.entries()) {
            ...
            if (session.status === "open" && session.pendingSpent > 0n) {
                if (idleMs >= idleSettleMs) {
                    void settleUptoSession(config.store, config.facilitatorClient, id, "idle_timeout", false, deadlineBufferSec);
                    continue;
                }
                if (timeToDeadline <= BigInt(deadlineBufferSec)) {
                    void settleUptoSession(config.store, config.facilitatorClient, id, "deadline_buffer", true, deadlineBufferSec);
                    continue;
                }
                if (totalOutstanding * capThresholdDen >=
                    session.cap * capThresholdNum) {
                    void settleUptoSession(config.store, config.facilitatorClient, id, "cap_threshold", false, deadlineBufferSec);
                    continue;
                }
            }
            if (idleMs >= longIdleCloseMs ||
                ...
                if (session.pendingSpent > 0n && session.status === "open") {
                    void settleUptoSession(config.store, config.facilitatorClient, id, "auto_close", true, deadlineBufferSec);
                }
                else {
